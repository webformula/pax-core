...
export default async function ({ rootFolder, distFolder = 'dist', pagesFolder, layoutFilePath, routerConfig, copyFiles = ([] = [{ from: undefined, to: undefined }]), serviceWorker = ({ } = { include: false, autoReload: true }) }, { customHTMLElementExtendedName, includeEntry = true, includeIndexHTML = true, includeRouter = true, includePage, includeTags, includesHTMLExtended } = {}) {
  ...
  if (serviceWorker && serviceWorker.include) {
    ...
    const cacheFiles = ['/', ...coreFiles, ...pagePaths, ...componentPaths, cssPath, ...copiedFilePaths];
