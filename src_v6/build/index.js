...
export default async function ({ rootFolder, distFolder = 'dist', pagesFolder, layoutFilePath, routerConfig, cacheBust = false, css = ({ } = { concat: false, filename: 'app.css' }), copyFiles = ([] = [{ from: undefined, to: undefined }]), serviceWorker = ({ } = { include: false, autoReload: true, cacheFiles: undefined }) }, { customHTMLElementExtendedName, includeEntry = true, includeIndexHTML = true, includeRouter = true, includePage, includeTags, includesHTMLExtended } = {}) {
  ...
  const cssPath = css.concat ? await concatCSS({ rootFolder, distFolder, filename: css.filename, componentCSSFiles }) : undefined;
